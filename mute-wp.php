...
add_action( 'admin_init', 'register_mwpn_styles' );
 ...
 */
function register_mwpn_styles() {
	wp_register_script( 'mwpn_scripts', plugins_url( 'mute-wp/js/mwpn_scripts.js' ) );
	wp_enqueue_script( 'mwpn_scripts' , array('jquery'));
}
...
function mwpn_disable_plugin_update_notification(){
			...
			echo '<script>localStorage.setItem("mute_plugin_update", "true")</script>';
...
function mwpn_enable_plugin_update_notification(){
			...
			echo '<script>localStorage.setItem("mute_plugin_update", "false")</script>';	

			wp_dequeue_script( 'mwpn_scripts' );
}
...
function remove_core_updates (){
	...
	/* TODO add functionality to remove core updates themselves (not only notifications) */
